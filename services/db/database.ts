...
async function migrateDatabase(db: SQLite.SQLiteDatabase): Promise<void> {
  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    await runInExclusiveTransaction(db, async (txn) => {
      if (currentVersion === 0) {
        // Fresh database: create all tables and indexes directly
        await txn.execAsync(`
          ...
          CREATE INDEX IF NOT EXISTS idx_transactions_source_goal ON transactions (source_goal_id);
          CREATE INDEX IF NOT EXISTS idx_contributions_goal ON goal_contributions (goal_id);
          CREATE INDEX IF NOT EXISTS idx_contributions_tx ON goal_contributions (transaction_id);
        ...
        // Incremental Migration for Version 3 (Category Budgets & Goal Linkages)
        if (currentVersion < 3) {
          try {
            await txn.execAsync(`
