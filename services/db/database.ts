...
export async function getDatabase(): Promise<SQLite.SQLiteDatabase> {
  if (!dbInstance) {
    ...
    await configurePragmas(dbInstance);
  }
  return dbInstance;
}
...
/**
 * Schema migrations for PiggySavings local-first SQLite DB.
 */
  ...
  await db.withTransactionAsync(async () => {
    ...
    await db.execAsync(`
      CREATE TABLE IF NOT EXISTS goals (
        ...
        icon_name TEXT,
        icon_family TEXT,
        card_variant TEXT DEFAULT 'card',
        created_at TEXT NOT NULL,
    ...
    await db.execAsync(`
      CREATE TABLE IF NOT EXISTS allocation_rules (
        ...
        goal_id TEXT NOT NULL,
        category_id TEXT,
        rule_type TEXT NOT NULL, -- 'percentage' | 'fixed_cents' | 'remainder'
        value REAL NOT NULL,
        min_income_cents INTEGER DEFAULT 0,
        is_active INTEGER DEFAULT 1,
        FOREIGN KEY (goal_id) REFERENCES goals (id) ON DELETE CASCADE,
    ...
    await db.execAsync(`
      CREATE TABLE IF NOT EXISTS user_entitlements (
        id TEXT PRIMARY KEY,
        unlocked_goal_slots INTEGER DEFAULT 3,
